...
def _tag_obs(obs, env_idx):
    item = dict(obs)
    item["_semantic_env_idx"] = int(env_idx)
    return item


def _stage_state_obs(obs, env_idx):
    """Strip unused images from intra-chunk semantic FSM updates."""
    return {"state": obs["state"], "_semantic_env_idx": int(env_idx)}
...
def eval_one_episode(TASK_ENV, model_client):
    ...
    while not TASK_ENV.is_episode_end():
        obs = _tag_obs(TASK_ENV.get_obs(), 0)
        ...
        for action_idx, action in enumerate(actions):
            ...
            if TASK_ENV.is_episode_end() or action_idx + 1 == len(actions):
                break
            model_client.call(
                func_name="update_stage_state",
                obs=_stage_state_obs(TASK_ENV.get_obs(), 0),
            )
...
def eval_one_episode_batch(TASK_ENV, model_client):
    ...
    while not TASK_ENV.is_episode_end():
        ...
        obs_list = [
            _tag_obs(obs, env_idx)
            for obs, env_idx in zip(TASK_ENV.get_obs_batch(env_idx_list), env_idx_list)
        ]
        ...
        for action_idx in range(chunk_size):
            ...
            obs_list = [
                _stage_state_obs(obs, env_idx)
                for obs, env_idx in zip(TASK_ENV.get_obs_batch(env_idx_list), env_idx_list)
            ]
            model_client.call(func_name="update_stage_state_batch", obs_list=obs_list)
