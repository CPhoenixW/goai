...
class Qwen3VLModel(Qwen3VLPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.action_embed = nn.Embedding(ACTION_MAX_LENGTH, config.text_config.hidden_size)
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if (input_ids >= STATE_ID).any():
            state_mask = (input_ids == STATE_ID).unsqueeze(-1).expand_as(inputs_embeds).to(inputs_embeds.device)
            inputs_embeds = inputs_embeds.masked_scatter(state_mask, state_embeds)

            action_mask = input_ids > STATE_ID
            action_input_ids = input_ids[action_mask]
            action_embeds = self.action_embed(action_input_ids - STATE_ID - 1)
            action_mask = action_mask.unsqueeze(-1).expand_as(inputs_embeds).to(inputs_embeds.device)
            inputs_embeds = inputs_embeds.masked_scatter(action_mask, action_embeds)

        if (input_ids == SCORE_ID).any():
            score_mask = (input_ids == SCORE_ID)
            score_input_ids = input_ids[score_mask]
            score_embeds = self.score_embed(score_input_ids - SCORE_ID)
            score_mask = score_mask.unsqueeze(-1).expand_as(inputs_embeds).to(inputs_embeds.device)
            inputs_embeds = inputs_embeds.masked_scatter(score_mask, score_embeds)
...
class Qwen3VLForConditionalGeneration(Qwen3VLPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        image_grid_thw: Optional[torch.LongTensor] = None,
        video_grid_thw: Optional[torch.LongTensor] = None,
        state_embeds: Optional[torch.LongTensor] = None,
        cache_position: Optional[torch.LongTensor] = None,
