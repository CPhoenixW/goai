...
评测范围：基于 X-Eval 仿真与真实一体化评测平台，面向 12 个仿真任务与 6 个真实机器人操作任务，关注任务成功率、泛化能力、执行效率与鲁棒性。

当前实现重点：RoboDojo 仿真侧 Generalization、`action_type=ee`

## 1. 任务定义

本项目对应方向一“通用双臂协作操作能力测试”，面向 X-Eval 仿真与真实一体化评测中的复杂双臂机器人操作场景。当前仓库实现的是仿真侧 RoboDojo 任务链路：输入多视角图像、机器人状态和任务指令，输出连续末端执行器动作，重点处理标准配置与 `*_random` 配置之间的分布差异。

竞赛目标覆盖 12 个仿真操作任务与 6 个真实机器人操作任务，并从任务成功率、泛化能力、执行效率和鲁棒性等维度进行综合优化。当前仓库的代码与视频证据覆盖仿真侧；真实机器人部署和 6 个真实任务结果不包含在本次仓库材料中。
