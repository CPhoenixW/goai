...
def build_model(cfg, model_path):
    ...
    supported = set(inspect.signature(XR1.__init__).parameters) - {"self"}
    ignored = sorted(set(model_cfg) - supported)
    if ignored:
        print(f"[deploy] Ignoring checkpoint-only XR1 parameters: {ignored}")
    model_cfg = {key: value for key, value in model_cfg.items() if key in supported}
    ...
    if isinstance(checkpoint, dict) and isinstance(checkpoint.get("module"), dict):
        ...
        print("[deploy] Unwrapped DeepSpeed checkpoint state from key 'module'")
    elif isinstance(checkpoint, dict) and isinstance(checkpoint.get("state_dict"), dict):
        state_dict = checkpoint["state_dict"]
        print("[deploy] Unwrapped Lightning checkpoint state from key 'state_dict'")
    else:
